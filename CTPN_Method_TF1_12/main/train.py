...
tf.app.flags.DEFINE_integer('max_steps', 50000, '') # 50000
tf.app.flags.DEFINE_integer('decay_steps', 30000, '')
tf.app.flags.DEFINE_float('decay_rate', 0.1, '')
tf.app.flags.DEFINE_float('moving_average_decay', 0.997, '')
tf.app.flags.DEFINE_integer('num_readers', 4, '') # 4
...
tf.app.flags.DEFINE_boolean('restore', False, '') # False
...
def main(argv=None):
    ...
    with tf.device('/gpu:%d' % gpu_id):
        with tf.name_scope('model_%d' % gpu_id) as scope:
            ...
            grads = opt.compute_gradients(total_loss) # optimizer calcultes 38 gradients
    ...
    with tf.control_dependencies([variables_averages_op, apply_gradient_op, batch_norm_updates_op]):
        train_op = tf.no_op(name='train_op') # a placeholder
