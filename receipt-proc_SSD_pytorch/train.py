...
# Data parameters
data_folder = 'updated_task1train_626p/'  # folder with data files
...
def main():
    ...
    if checkpoint is None:
        ...
        print(epochs_since_improvement)
        try:
            epochs_since_improvement = checkpoint['epochs_since_improvement'] 
        except KeyError:
            print("epochs_since_improvement Key not in checkpoint dictionary")
        try:
            best_loss = checkpoint['best_loss'] + 1
        except KeyError:
            print("best_loss Key not in checkpoint dictionary")
        print('\nLoaded checkpoint from epoch %d. Best loss so far is %.3f.\n' % (start_epoch, best_loss))
        model = checkpoint['model']
        try:
            optimizer = checkpoint['optimizer']
        except KeyError:
            print("Optimizer not yet in checkpoint dictionary")
            optimizer = torch.optim.Adam(model.parameters(), lr=lr, betas=(0.9, 0.99)) # copied frm ln 59 to initialize with something
