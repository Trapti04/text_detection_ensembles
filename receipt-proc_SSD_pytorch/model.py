        ...
        for i in range(batch_size):
            ...
            for c in range(1, self.n_classes):
                ...
                for box in range(class_decoded_locs.size(0)):
                    ...
                    suppress_ = torch.max(suppress, condition)
                    suppress = suppress_
            ...
            print("OriginalImage", original_image)
            ...
            for box in range(det_boxes.size(0)):
                cond = overlap[box] > max_OCR_overlap
                cond = torch.tensor(cond, dtype=torch.uint8).to(device)
                suppress__ = torch.max(suppress, cond)
                suppress = suppress__
                suppress[box] = 0
...
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, priors_cxcy, threshold=0.5, neg_pos_ratio=3, alpha=1.):
        ...
        #self.priors_xy = self.clearcxcy_to_xy(priors_cxcy) # commented by Trapti due to error
